# 2.10.3 WeakSet �?WeakMap

## 概述

WeakSet �?WeakMap �?ES6 引入的弱引用集合类型。它们与 Set �?Map 的主要区别在于：WeakSet �?WeakMap 中的键（对于 WeakSet 是值）必须是对象，并且这些对象是弱引用的，不会阻止垃圾回收�?

WeakSet �?WeakMap 的主要特点：
- **弱引�?*：不会阻止垃圾回收器回收对象
- **只能存储对象**：键（WeakSet 的值）必须是对象类�?
- **不可迭代**：没�?size 属性，不能遍历
- **不可枚举**：没�?keys()、values()、entries() 等方�?
...
**返回�?*：布尔值，如果 WeakSet 中包含该对象返回 `true`，否则返�?`false`
...
**返回�?*：布尔值，如果删除成功返回 `true`，如果对象不存在返回 `false`
...
**参数**�?
- `iterable`（可选）：可迭代对象，每个元素应该是 `[key, value]` 形式的数组，其中 key 必须是对�?
...
**参数**�?
- `key`（必需）：要添加的键，必须是对象类�?
- `value`（必需）：要添加的值，可以是任何类�?

**返回�?*：WeakMap 对象本身（支持链式调用）
...
**返回�?*：布尔值，如果 WeakMap 中包含该键返�?`true`，否则返�?`false`
...
class User {
    ...
    // 无法直接访问 password
    // �?User 实例被垃圾回收时，WeakMap 中的私有数据也会被自动清�?
}
...
// 当按钮从 DOM 中移除时，WeakMap 中的元数据会自动被垃圾回�?
```
...
| **值类�?*     | 只能是对�?                | 值可以是任何类型           |
| **使用场景**   | 标记对象、防止循环引�?    | 私有数据、元数据、缓�?    |
| **方法**       | add、has、delete           | set、get、has、delete      |
| **共同�?*     | 弱引用、不可迭代、无 size  | 弱引用、不可迭代、无 size  |

### 选择建议

```js
// 使用 WeakSet：只需要标记对象是否存�?
const processed = new WeakSet();
if (!processed.has(obj)) {
    processed.add(obj);
}
...
WeakSet �?WeakMap 只能存储对象，不能存储原始值：
...
WeakSet �?WeakMap 没有迭代器，不能使用 for...of 循环�?
...
WeakSet �?WeakMap 没有 clear() 方法，不能一次性清空：
...
- **内存管理**：WeakSet �?WeakMap 的主要优势是自动内存管理
- **查找性能**：has() �?get() 方法的时间复杂度�?O(1)
- **垃圾回收**：弱引用不会阻止垃圾回收，有助于防止内存泄漏

## 常见问题

### 问题 1：为什�?WeakSet �?WeakMap 不能迭代�?

WeakSet �?WeakMap 使用弱引用，对象的生命周期不确定。如果允许迭代，可能会在迭代过程中遇到已被垃圾回收的对象，导致不一致的行为�?

### 问题 2：如何检�?WeakSet �?WeakMap 是否为空�?

WeakSet �?WeakMap 没有 size 属性，无法直接检查是否为空。如果需要这个功能，应该使用 Set �?Map�?

### 问题 3：WeakSet �?WeakMap 适合什么场景？

WeakSet �?WeakMap 适合以下场景�?
- 需要存储对象的元数据，但不希望影响对象的垃圾回�?
- 需要标记对象，但标记信息是临时�?
- 需要实现真正的私有属�?
- 需要存�?DOM 元素的关联数�?

### 问题 4：如何清�?WeakSet �?WeakMap�?

WeakSet �?WeakMap 没有 clear() 方法。如果需要清空，需要重新创建实例，或者让所有键对象失去引用，等待垃圾回收�?
...
function processObject2(obj) {
    ...
    processedSet.add(obj); // 会阻�?obj 被垃圾回�?
}
```

### 3. 使用 WeakMap 存储 DOM 元数�?

```js
// 好的做法：使�?WeakMap 存储 DOM 元数�?
const elementData = new WeakMap();
...
// 但可以确定的是，如果没有其他引用，对象会被回�?
```

## 练习

1. **WeakSet 标记对象**：使�?WeakSet 实现一个对象处理系统，标记已处理的对象，避免重复处理�?

2. **WeakMap 私有属�?*：使�?WeakMap 实现一个类，将私有数据存储�?WeakMap 中，实现真正的私有属性�?

3. **DOM 元素元数�?*：使�?WeakMap 存储 DOM 元素的元数据（如点击次数、最后点击时间等），当元素被移除时自动清理�?

4. **缓存系统**：使�?WeakMap 实现一个对象计算结果的缓存系统，当对象被垃圾回收时自动清理缓存�?

5. **对象关联**：使�?WeakMap 存储对象之间的关联关系，实现对象之间的弱引用关联�?

完成以上练习后，继续学习下一章：面向对象与原型�?

## 总结

WeakSet �?WeakMap 提供了弱引用的集合功能。主要要点：

- WeakSet：对象的弱引用集合，用于标记对象
- WeakMap：键值对的弱引用集合，键必须是对�?
- 弱引用：不会阻止垃圾回收，有助于防止内存泄漏
- 不可迭代：没�?size 属性，不能遍历
- 使用场景：私有数据、DOM 元数据、对象标记、缓�?
- 只能存储对象：不能存储原始�?
- 注意弱引用的影响和不可迭代的限制
