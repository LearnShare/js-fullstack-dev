...
ES2024（ES15）是 ECMAScript 2024 标准，于 2024 �?6 月发布。ES2024 引入�?Promise.withResolvers、正则表达式 /v 标志、Object.groupBy 等特性，进一步完善了 JavaScript 的异步编程和数据处理能力�?
## 核心特�?
ES2024 主要包含以下新特性：

1. **Promise.withResolvers()**：创建可外部控制�?Promise
2. **Object.groupBy()**：按条件分组对象数组
3. **Array.prototype.groupBy()**：按条件分组数组元素
4. **正则表达�?/v 标志**：可编排正则模式（Unicode 属性转义改进）
...
`Promise.withResolvers()` 返回一个对象，包含一个新�?Promise 及其 resolve �?reject 函数，允许在 Promise 外部控制其状态�?
### 语法格式
...
| `resolve`   | function | 解决 Promise 的函�?     |
| `reject`    | function | 拒绝 Promise 的函�?     |
...
**输出说明**：在事件监听器中，可以使�?`resolve` 函数手动解决 Promise�?
### 与传�?Promise 构造函数的对比
...
2. **兼容�?*：需�?Node.js 22.0.0+ 或现代浏览器支持
3. **最佳实�?*：避免过度使用，优先使用标准�?Promise 构造函�?
### 常见错误

**错误 1**：多次调�?resolve �?reject

```js
const { promise, resolve, reject } = Promise.withResolvers();

resolve('First');
resolve('Second'); // 无效，Promise 已解�?
promise.then(value => {
...
| 参数�?      | 类型     | 说明                           | 是否必需 | 默认�?|
|:-------------|:---------|:-------------------------------|:---------|:-------|
| `items`      | iterable | 要分组的可迭代对�?            | �?      | -      |
| `callbackFn` | function | 为每个元素调用的函数           | �?      | -      |
...
**输出说明**：按奇偶性分组，返回包含 'odd' �?'even' 键的对象�?
### 注意事项

1. **返回对象**：返回一个普通对象，不是 Map
2. **键的类型**：键会被转换为字符串
3. **兼容�?*：需�?Node.js 22.0.0+ 或现代浏览器支持
...
### �?Object.groupBy() 的对�?
| 特�?             | `Object.groupBy()`        | `Array.prototype.groupBy()` |
|:------------------|:--------------------------|:---------------------------|
...
1. **Babel**：使用相应的插件转译
2. **TypeScript**：在 `tsconfig.json` 中设�?`target: "ES2024"`
3. **Polyfill**：可以使�?`core-js` �?polyfill

---

## 最佳实�?
1. **Promise.withResolvers()**�?   - 用于需要在 Promise 外部控制其状态的场景
   - 避免过度使用，优先使用标�?Promise 构造函�?
2. **分组方法**�?   - 使用 `Object.groupBy()` 处理任何可迭代对�?   - 使用 `Array.prototype.groupBy()` 处理数组（更简洁）

---

## 练习

1. **Promise.withResolvers()**�?   - 创建一个函数，使用 `Promise.withResolvers()` 实现超时控制
   - 在事件监听器中使�?`Promise.withResolvers()` 等待用户交互

2. **分组方法**�?   - 使用 `Object.groupBy()` 将用户数组按年龄分组
   - 使用 `Array.prototype.groupBy()` 将数字数组按奇偶性分�?
3. **综合练习**�?   - 实现一个数据统计函数，使用分组方法分析数据分布
...
- **Promise.withResolvers()**：提供更灵活�?Promise 控制机制
- **分组方法**：简化数据分组操�?
