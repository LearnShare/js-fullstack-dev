...
`this` 表示函数运行时的上下文对象。它的指向不取决于定义位置，而取决于“调用方式”。掌�?`this` 对象能避免常�?Bug（丢失上下文、事件回调指向错误），也是理解面向对象与异步回调的重要基础。本节系统讲�?`this` 的绑定规则、各场景示例、常见陷阱与最佳实践�?

## this 绑定的四条核心规�?

> 决定 `this` 的是“调用点”，而非“定义点”。按照优先级从高到低�?
> 1) new 绑定 2) 显式绑定 3) 隐式绑定 4) 默认绑定；箭头函数不适用这四条，会继承外�?`this`（词法绑定）�?

## 1. 默认绑定

非严格模式下，独立函数调�?`this` 指向全局对象（浏览器�?`window`，Node �?`global`）；严格模式下为 `undefined`�?
...
const u = new obj.User('Tom'); // this -> 新实�?
```

## 5. 箭头函数的词法绑�?

箭头函数没有自己�?`this`，会捕获定义时所在词法作用域�?`this`�?
...
const obj = {
  ...
  greet() {
    setTimeout(() => {
      console.log(this.name); // Eve，继�?obj �?this
    }, 10);
  }
};
obj.greet();
...
```
�?class 组件/对象方法中，为避免丢失上下文，常�?`bind` 或箭头函数封装回调�?
...
class Counter {
  count = 0;
  inc = () => { this.count += 1; }; // 字段箭头函数，绑定外�?this（实例）
}
```
...
### 数组方法回调中的 thisArg
大多数数组方法（map/filter/forEach）可传�?`thisArg` 作为回调�?`this`�?
```js
const obj = { factor: 2 };
...
- 默认绑定：严格模式下 `this` �?`undefined`，有助于发现错误�?
...
1) 对象方法用普通函数；内部回调用箭头函数保持外�?`this`�? 
2) 类方法若会被当作回调传递，构造函数里 `bind`，或声明为箭头函数字段�? 
3) 事件监听使用普通函数获取目标元素的 `this`；需要闭包时用箭头函数或捕获参数�? 
4) 在严格模�?ESM 下编写，避免默认绑定落到全局�? 
5) 避免在模块顶层依�?`this`（ESM 顶层 this �?`undefined`）�? 
6) 需要动态上下文时优先使�?`call`/`apply`/`bind`，避免隐式丢失�? 

## 练习

1. 编写一个对象方法，演示隐式绑定与隐式丢失，并用 `bind` 修正�? 
2. 将类方法传入 `setTimeout`，分别展示未绑定与使用箭头函�?`bind` 的差异�? 
3. 在数�?`map` 中传�?`thisArg`，实现带权重的运算�? 
4. 用事件监听分别尝试普通函数与箭头函数，观�?`this` 指向的不同�? 
...
- 回调、事件、类方法传递时最易丢�?`this`，使�?`bind` 或箭头函数封装�? 
- 充分理解并按场景选用绑定方式，可显著降低上下文相�?Bug�? 
