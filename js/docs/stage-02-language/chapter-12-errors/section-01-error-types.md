...
```
- 场景：调用非函数、解�?null/undefined、访问不存在的方法等�?
...
排查建议：检查数据类型、提前做空值守卫、确�?this 绑定�?

### ReferenceError 常见触发
- 变量未声�?/ 拼写错误
- 暂时性死区：�?`let/const` 声明前访�?

排查建议：启�?linter，检查作用域，避免在声明前使用�?
...
排查建议：确保输入已正确编码；对用户输入�?try/catch�?
...
- **可预期业务错�?*：输入校验失败、权限不�?�?返回业务码；不应上报为致命错误�?
- **可恢复错�?*：网络重试、缓存降级�?
- **编程错误**：空指针、类型错误、语法错�?�?开发期修复，生产期兜底上报+降级�?

## 最佳实�?

1) 使用语义化自定义错误，附�?`code`/`field`/`status`�? 
2) 对外暴露友好提示，对内保留详�?message/stack�? 
3) 在关键边界做参数校验，减�?TypeError/RangeError�? 
4) �?JSON/动态构造代码做 try/catch，避�?SyntaxError 崩溃�? 
...
2. 写一�?JSON 解析函数，捕�?SyntaxError 并返回安全结果�? 
3. 实现一�?ValidationError，并在表单校验中使用�? 
4. �?`Promise.any` 构�?AggregateError，遍�?`errors` 输出�? 
5. 在浏览器�?Node 分别触发同类错误，比�?stack 格式差异�? 
...
继续学习下一节，了解异常捕获�?try...catch...finally�?
