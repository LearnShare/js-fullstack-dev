# 2.13.1 JavaScript 语法指南

## 概述

JavaScript 语法指南定义了编写 JavaScript 代码时应遵循的基本语法规则，包括变量声明、语句格式、代码组织等。遵循统一的语法规范有助于提高代码可读性和可维护性。
...
2. **可读性**：代码应该易于阅读和理解
3. **简洁性**：避免不必要的复杂性
4. **现代性**：优先使用现代 JavaScript 语法（ES2015+）
...
**说明**：`const` 和 `let` 具有块级作用域，避免变量提升问题。
...
// 推荐：导入
import User, { PI, calculateArea } from './math.js';
```
...
**推荐**：使用 `async/await` 替代 Promise 链
...
1. **严格模式**：在模块和类中自动启用严格模式
2. **分号**：根据团队规范统一使用或不使用分号
3. **引号**：统一使用单引号或双引号（推荐单引号）
4. **缩进**：使用 2 个空格或 4 个空格（推荐 2 个空格）

---

## 最佳实践

1. **使用现代语法**：优先使用 ES2015+ 特性
2. **保持一致性**：在整个项目中保持统一的代码风格
3. **工具辅助**：使用 ESLint、Prettier 等工具自动格式化
4. **代码审查**：通过代码审查确保规范执行
