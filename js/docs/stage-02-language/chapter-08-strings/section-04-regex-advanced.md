...
使用 `\p{...}` 匹配 Unicode 属性，需要 `u` 标志：

```js
// 匹配所有字母（包括中文、日文等）
const re = /\p{Letter}+/gu;
const str = 'Hello 世界 こんにちは';
console.log(str.match(re)); // ["Hello", "世界", "こんにちは"]
```
...
从 `lastIndex` 位置开始匹配，必须连续：

```js
const re = /\d+/y;
re.lastIndex = 5;
console.log(re.test('123 456')); // false（位置5是空格）

re.lastIndex = 4;
console.log(re.test('123 456')); // true（位置4是4）
```

### 与 g 标志的区别

```js
// g 标志：全局搜索，可以跳过不匹配的字符
const reG = /\d+/g;
const str = 'a1b2c3';
console.log(str.match(reG)); // ["1", "2", "3"]

// y 标志：必须从 lastIndex 开始连续匹配
const reY = /\d+/y;
reY.lastIndex = 1;
console.log(reY.exec(str)); // ["1"]
reY.lastIndex = 2;
console.log(reY.exec(str)); // null（位置2是'b'，不匹配）
```
...
```js
// 提取并清理数据
const text = 'Prices: $99.99, $199.99, $299.99';
const prices = text
    .match(/\$\d+\.\d{2}/g)           // 提取价格
    .map(price => price.replace('$', '')) // 移除 $ 符号
    .map(Number);                      // 转换为数字
console.log(prices); // [99.99, 199.99, 299.99]
```
...
1. **命名捕获组兼容性**：ES2018+，较旧的浏览器可能不支持
2. **后向断言兼容性**：ES2018+，需要较新的 JavaScript 引擎
3. **Unicode 属性转义**：必须使用 `u` 标志，ES2018+
4. **性能考虑**：复杂正则可能影响性能，需要优化
5. **可读性**：命名捕获组可以提高代码可读性
...
1. **使用命名捕获组**：提高代码可读性，特别是复杂正则
2. **合理使用断言**：避免复杂的断言嵌套，保持可读性
3. **Unicode 属性转义**：处理多语言文本时使用，记住加 `u` 标志
4. **性能优化**：对于性能敏感的场景，使用原子组减少回溯
5. **注释说明**：复杂正则添加注释，说明各部分的作用

## 练习

1. **命名捕获组练习**：编写正则表达式提取邮箱地址，使用命名捕获组分别提取用户名和域名。

2. **后向断言练习**：编写正则表达式匹配价格（如 "$99.99"），但只返回数字部分，使用后向断言。

3. **Unicode 属性练习**：编写正则表达式统计文本中 emoji 的数量。

4. **反向引用练习**：编写正则表达式检测字符串中是否有重复的单词（如 "the the"）。

5. **综合练习**：编写正则表达式验证并提取 URL，使用命名捕获组分别提取协议、域名和路径。
