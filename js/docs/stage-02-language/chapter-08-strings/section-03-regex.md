...
正则表达式用于文本的匹配、查找、提取与替换。掌握模式语法（字符类、量词、分组、边界）、标志位、常用 API，以及避免回溯灾难的写法，可帮助你编写高效可维护的文本处理逻辑。
...
```js
const re = /pattern/;
const re2 = /pattern/gi; // g: 全局, i: 忽略大小写
```
...
- `m` 多行（`^`/`$` 匹配行首/行尾）
- `s` 允许 `.` 匹配换行（dotAll）
- `u` Unicode，启用 Unicode 感知（推荐处理多语言/emoji）
...
| 参数名   | 类型   | 说明                           | 是否必需 | 默认值 |
|:---------|:-------|:-------------------------------|:---------|:-------|
| `str`    | string | 要测试的字符串                 | 是       | -      |

**返回值**：布尔值，如果正则表达式匹配字符串返回 `true`，否则返回 `false`
...
| 参数名     | 类型         | 说明                           | 是否必需 | 默认值 |
|:-----------|:-------------|:-------------------------------|:---------|:-------|
| `regexp`   | RegExp\|string | 正则表达式对象或字符串         | 是       | -      |

**返回值**：
- 如果使用全局标志（`g`），返回匹配结果的数组，如果没有匹配返回 `null`
- 如果不使用全局标志，返回第一个匹配结果及其捕获组，如果没有匹配返回 `null`
...
| 参数名     | 类型         | 说明                           | 是否必需 | 默认值 |
|:-----------|:-------------|:-------------------------------|:---------|:-------|
| `regexp`   | RegExp       | 正则表达式对象（必须使用全局标志 `g`） | 是       | -      |

**返回值**：返回一个迭代器，包含所有匹配结果及其详细信息（索引、捕获组等）
...
**返回值**：数字，返回第一个匹配的索引位置，如果没有匹配返回 `-1`
...
| 参数名          | 类型         | 说明                           | 是否必需 | 默认值 |
|:----------------|:-------------|:-------------------------------|:---------|:-------|
| `regexp\|substr` | RegExp\|string | 正则表达式或要替换的子字符串   | 是       | -      |
| `newSubstr\|function` | string\|Function | 替换字符串或替换函数 | 是       | -      |

**返回值**：字符串，返回替换后的新字符串（原字符串不变）

**说明**：
- 如果第一个参数是字符串，只替换第一个匹配
- 如果第一个参数是正则表达式且使用全局标志（`g`），替换所有匹配
...
| 参数名          | 类型         | 说明                           | 是否必需 | 默认值 |
|:----------------|:-------------|:-------------------------------|:---------|:-------|
| `regexp\|substr` | RegExp\|string | 正则表达式（必须使用全局标志 `g`）或要替换的子字符串 | 是       | -      |
| `newSubstr\|function` | string\|Function | 替换字符串或替换函数 | 是       | -      |
...
| 参数名        | 类型         | 说明                           | 是否必需 | 默认值 |
|:--------------|:-------------|:-------------------------------|:---------|:-------|
| `separator`   | RegExp\|string | 分隔符（正则表达式或字符串）   | 否       | -      |
...
| 参数名   | 类型   | 说明                           | 是否必需 | 默认值 |
|:---------|:-------|:-------------------------------|:---------|:-------|
| `str`    | string | 要匹配的字符串                 | 是       | -      |

**返回值**：
- 如果匹配成功，返回匹配结果数组（包含捕获组），并更新 `lastIndex` 属性
- 如果匹配失败，返回 `null`
...
- 匹配任意字母：`\p{L}`（需 `u`）。
- 匹配 emoji/扩展字符时建议使用 `u`，避免 surrogate pair 被拆分。
...
- 用非捕获分组、明确边界、合理量词上限。
- 大文本重复匹配使用 `y`（粘连）或手动推进 `lastIndex`，减少回溯。
...
- 未使用 `u` 处理 Unicode，导致 emoji 匹配异常。  
- 使用贪婪量词导致匹配过长；需要时加 `?` 改为惰性。  
- 复杂正则缺少注释/分段，后期难维护。可使用 `x` 风格（在 JS 中可拆分字符串加注释）。  

## 最佳实践

1) 对用户输入动态构造正则前先转义。  
2) 处理多语言/emoji 时加 `u` 标志，使用 `\p{...}`。  
3) 优先非捕获分组 `(?:...)`，仅在需要结果时使用捕获组。  
...
1. 写一个手机号匹配正则（带国家码可选），并用 `test` 验证。  
2. 用 `matchAll` 提取文本中的所有链接及其起始索引。  
3. 将用户输入的关键字转义后高亮显示（使用 `replace` + 回调）。  
4. 处理包含 emoji 的文本，统计 emoji 数量（使用 `\p{Emoji}`，需 `u`）。  
