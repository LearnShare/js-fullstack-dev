...
- JS 字符串基于 UTF-16 编码，每个“代码单元”占 2 字节。
- `length` 统计的是代码单元数量，不一定等于实际“字符”数（对代理对/emoji 会大于 1）。
- 常见长度差异：
```js
const s = '😊';        // 单个 emoji
...
不推荐使用 `new String()`，它返回对象而非原始值，在比较/类型判断时易混淆。
...
// 代码单元（16 位）转字符串
const char = String.fromCharCode(0x4F60); // '你'
```
...
应用：XSS 防护、国际化格式化、样式构造（如 styled-components）。
...
本地化敏感操作使用 `toLocaleUpperCase`/`toLocaleLowerCase`。
```js
'istanbul'.toLocaleUpperCase('tr'); // 处理土耳其语 i
```

## 常见陷阱

- 误用 `new String()` 导致类型为 object，`===` 比较为 false。
- `length` 对 emoji/代理对不等于实际字符数。
- 模板字符串内无意的换行/空格导致格式问题。
...
1. 写一个标签模板函数 `escapeHTML`，对插值做 HTML 转义。  
2. 统计包含 emoji 的字符串真实字符数（按码点），并安全截取前 N 个字符。  
3. 使用模板字符串生成多行 Markdown 片段，去除多余缩进。  
4. 演示 `new String()` 与字面量在 `typeof`、`===` 的差异。  
5. 编写一个安全的 `format` 函数，支持占位 `${}` 与默认值。  

## 总结

- 字符串基于 UTF-16，长度为代码单元数，处理补充平面字符需注意码点。  
