...
| 参数名         | 类型   | 说明                           | 是否必需 | 默认值 |
|:---------------|:-------|:-------------------------------|:---------|:-------|
| `searchValue`  | string | 要查找的子字符串               | 是       | -      |
| `fromIndex`    | number | 开始查找的索引位置             | 否       | 0      |

**返回值**：数字，第一个匹配的索引，未找到返回 -1
...
| 参数名         | 类型   | 说明                           | 是否必需 | 默认值 |
|:---------------|:-------|:-------------------------------|:---------|:-------|
| `searchString` | string | 要查找的子字符串               | 是       | -      |
| `position`     | number | 开始查找的索引位置             | 否       | 0      |
...
| 参数名         | 类型   | 说明                           | 是否必需 | 默认值 |
|:---------------|:-------|:-------------------------------|:---------|:-------|
| `searchString` | string | 要查找的前缀字符串             | 是       | -      |
| `position`     | number | 开始查找的索引位置             | 否       | 0      |

**返回值**：布尔值，以指定字符串开头返回 `true`，否则返回 `false`
...
| 参数名         | 类型   | 说明                           | 是否必需 | 默认值 |
|:---------------|:-------|:-------------------------------|:---------|:-------|
| `searchString` | string | 要查找的后缀字符串             | 是       | -      |
| `length`       | number | 作为字符串长度的位置           | 否       | `str.length` |

**返回值**：布尔值，以指定字符串结尾返回 `true`，否则返回 `false`
...
| 参数名   | 类型   | 说明           | 是否必需 | 默认值 |
|:---------|:-------|:---------------|:---------|:-------|
| `regexp` | RegExp | 正则表达式     | 是       | -      |

**返回值**：数字，第一个匹配的索引，未匹配返回 -1
...
| 参数名        | 类型           | 说明                           | 是否必需 | 默认值 |
|:--------------|:---------------|:-------------------------------|:---------|:-------|
| `searchValue` | string\|RegExp | 要替换的字符串或正则表达式     | 是       | -      |
| `replacer`    | string\|function | 替换字符串或替换函数         | 是       | -      |

**返回值**：字符串，替换后的新字符串（不修改原字符串）
...
**注意事项**：`replaceAll()` 需要 ES2021+ 支持，旧环境使用 `/.../g` + `replace()`。
...
| 参数名        | 类型   | 说明                           | 是否必需 | 默认值     |
|:--------------|:-------|:-------------------------------|:---------|:-----------|
| `indexStart`  | number | 起始索引（包含）               | 是       | -          |
| `indexEnd`    | number | 结束索引（不包含）             | 否       | `length`   |

**返回值**：字符串，截取后的新字符串（不修改原字符串）

**说明**：不支持负数，会自动交换 start 和 end
...
**注意事项**：`substring()` 不支持负数索引，参数会自动交换，容易产生困惑。建议优先使用 `slice()`。

### substr()（已废弃）

**说明**：`substr()` 方法已废弃，不应使用。使用 `slice()` 替代。
...
| 参数名       | 类型           | 说明                           | 是否必需 | 默认值 |
|:-------------|:---------------|:-------------------------------|:---------|:-------|
| `separator`  | string\|RegExp | 分隔符                         | 否       | -      |
| `limit`      | number         | 限制返回数组的最大长度         | 否       | -      |
...
| 参数名         | 类型   | 说明                           | 是否必需 | 默认值 |
|:---------------|:-------|:-------------------------------|:---------|:-------|
| `targetLength` | number | 目标长度                       | 是       | -      |
| `padString`    | string | 填充字符串                     | 否       | ' '    |

**返回值**：字符串，在开头填充后的新字符串

**说明**：如果字符串长度小于目标长度，在开头填充
...
- 复杂大小写/折叠可用 `Intl.Collator`，多语言格式化可用 `Intl` 系列 API。

## 常见陷阱

- `substr` 已废弃，避免使用。  
- `substring` 会交换参数且不支持负数，易产生困惑；优先 `slice`。  
- 处理 emoji/补充平面字符时，`length`/`charAt`/`slice` 可能拆半字符；用展开或码点 API。  
...
2) 替换多个匹配时优先 `replaceAll` 或带 `/g` 的正则。  
3) 规范化大小写时，需考虑本地化：用 `toLocaleUpperCase`/`toLocaleLowerCase`。  
4) 优先 `slice` 处理区间；避免 `substr`，谨慎使用 `substring`。  
5) 需排序/比较的场景使用 `localeCompare` 或 `Intl.Collator`，避免简单 ASCII 比较导致多语言错误。  

## 练习

1. 写一个 `safeSlice(str, start, end)`，按码点截取，支持负数。  
2. 使用 `replaceAll` 或正则，将字符串中的 HTML 标签去除，保留文本。  
3. 用 `localeCompare` 对包含重音符的单词进行德语排序。  
4. 统计字符串中出现次数最多的字符，考虑 emoji 计数。  
5. 将 CSV 行用 `split` 转为数组，并去除多余空格。  
