...
**对齐说明**：
- 第一列"特性"：18 字符宽度，内容"开发服务器启动"63 字符，需要对齐到至少 63 字符位置
- 第二列"Vite"：4 字符宽度，内容"极快"18 字符，需要对齐到至少 18 字符位置
- 第三列"Webpack"：7 字符宽度，内容"中等"18 字符，需要对齐到至少 18 字符位置
...
**必须遵守**：
- 使用标准 Markdown 代码块格式：```` ```ts ````
- **禁止使用** CODE REFERENCES 格式（如 ````startLine:endLine:filepath）
- 代码块前后各留一空行
- **语言标识必须使用简写**：`ts`（不是 `typescript`）、`js`（不是 `javascript`）、`bash`（不是 `shell`）、`json`、`md` 等
